 ...
 * - Strict mode: requireKyber:true → returns policy error, no envelope
 *
 * Usage: npx tsx test/crypto-roundtrip.test.ts
 */

// pqc-shared is imported here ONLY for the decrypt side (simulating server).
// In the actual client bundle, pqc-shared is never loaded at module scope.
import {
  hybridEncrypt,
...
import type { HybridSecretKeys } from "@omnituum/pqc-shared";
...
async function testVectorB(): Promise<void> {
  ...
  assert(envelope.v === ENVELOPE_VERSION, `v = "${ENVELOPE_VERSION}"`);
  assert(envelope.aead === ENVELOPE_AEAD, `aead = "${ENVELOPE_AEAD}"`);
  assert(envelope.suite === "x25519", 'suite = "x25519"');
  assert(envelope.x25519Epk.length === 64, "x25519Epk is 64 hex chars");
  assert(envelope.x25519Wrap.nonce.length > 0, "x25519Wrap.nonce is non-empty");
  ...
  // THE CRITICAL TEST: hybridDecrypt can decrypt X25519-only envelopes
  const decrypted = await hybridDecryptToString(envelope as any, SECRET_KEYS);
